    ...
    return (
        <motion.div initial={{ opacity: 0, y: 50, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ type: "spring" }} className='relative w-full rounded-3xl overflow-hidden dark:bg-white/10'>
            <div className='absolute inset-0 z-0'>
                {tab.mediaArtwork && <Image loading='eager' src={tab.mediaArtwork} alt="source" width={1024} height={1024} className='w-full h-full object-cover opacity-40 blur-xl' />}
                <div className='absolute inset-0 bg-black/50' />
            </div>
            <motion.div className='p-4 flex gap-6 z-10 relative'>
                <motion.div className='relative border-2 border-zinc-800 aspect-square rounded-full flex items-center justify-center w-full max-w-24'>
                    {
                    ...
                    <div className='w-5 h-5 bg-black/80 border-2 border-zinc-800 absolute rounded-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2' />
                </motion.div>
                <motion.div className='flex gap-2 flex-col w-fit overflow-hidden'>
                    ...
                    <motion.div className={`flex gap-2 items-center text-xs ${tab.playback === 'PLAYING' ? 'text-green-500' : tab.playback === 'PAUSED' ? 'text-yellow-500' : 'text-red-500'}`}>
                    ...
                    <motion.div className={`flex gap-2 items-center text-xs ${tab.muted ? 'text-red-500' : 'text-white'}`}>
                        <Icons.volumeMute />
