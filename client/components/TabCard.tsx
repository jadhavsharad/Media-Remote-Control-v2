...
export const TabCard = ({ tab }: { tab: MediaTab }) => {
    return (
        <motion.div initial={{ opacity: 0, y: 50, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} transition={{ type: "spring" }} className='relative w-full rounded-3xl overflow-hidden dark:bg-white/10 border border-black/5'>
            ...
            <motion.div className='p-4 flex gap-6 z-10 relative'>
                <motion.div className='relative border-2  dark:border-zinc-800 border-zinc-300 aspect-square rounded-full flex items-center justify-center w-full max-w-24'>
                    {
                        tab.mediaArtwork ? <Image loading='eager' src={tab.mediaArtwork} alt={tab.mediaTitle || tab.title} width={1024} height={1024} className=' w-full h-full object-cover rounded-[inherit]' /> : <div className='absolute inset-0 rounded-[inherit] bg-zinc-200 dark:bg-zinc-800'></div>
                    }
                    <div className='w-5 h-5 dark:bg-black/80 bg-zinc-300 border-2 dark:border-zinc-800 border-zinc-300 absolute rounded-full top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2' />
                </motion.div>
